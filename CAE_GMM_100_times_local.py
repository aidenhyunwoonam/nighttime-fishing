sims_outfile = open("C:/Users/jseong/Desktop/hwnam/2026/lights2/step-2-CAE-GMM/0_CAE_Clustering_100times/local_results_0422/output100.txt", 'w')
#%%
...
for sim_run in range (1, 101):
    ...
    output_dir = r"C:/Users/jseong/Desktop/hwnam/2026/lights2/step-2-CAE-GMM/0_CAE_Clustering_100times/local_results_0422/"+ str(sim_run)
    ...
    # Normalize per image using only valid pixels
    images_norm = np.zeros_like(images, dtype="float32")
    for i in range(images.shape[0]):
        valid = masks[i].astype(bool)
        vals = images[i][valid]
        mean, std = (0.0, 1.0) if vals.size == 0 else (float(vals.mean()), float(vals.std()) if vals.std() > 0 else 1.0)
        images_norm[i] = (images[i] - mean) / std
        images_norm[i][~valid] = 0.0
